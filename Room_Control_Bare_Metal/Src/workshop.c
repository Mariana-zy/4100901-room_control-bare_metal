...
// --- Definiciones de regustros para Button B1 (Ver RM0351) -------------------
#define GPIOC_BASE  0x48000800U
#define GPIOC_MODER (*(volatile uint32_t *)(GPIOC_BASE + 0x00U)) // Configuración de modo
#define GPIOC_IDR   (*(volatile uint32_t *)(GPIOC_BASE + 0x10U)) // Data de salida
#define B1_PIN     13U                                           // Pin PC13 (LED)
...
int main(void)
{
    ...
    while (1) {
        ...
        if (GPIOC_IDR & (1 << B1_PIN)) {          // Si se presiona el boton
            GPIOA_ODR ^= (1 << LD2_PIN);          // Apaga LED
        }
    }
}
...
// --- Inicialización de GPIOC PC13 para el Button B1 ---------------------------
...
void check_button(void)
{
    // Leer estado del botón PC13 (botón presionado = 0, o presionado = 1)
